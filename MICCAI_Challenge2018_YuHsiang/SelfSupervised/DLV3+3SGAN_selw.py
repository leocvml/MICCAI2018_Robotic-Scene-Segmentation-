...
DissNet = nn.HybridSequential()
with DissNet.name_scope():
    DissNet.add(
        Discriminator()
    )
DissNet.initialize()
...
DissNet.collect_params().reset_ctx(ctx=ctx)
...
sDtrainer = gluon.Trainer(DissNet.collect_params(),'adam',{'learning_rate': 0.0002})

G_filename = name+'_G.params'
D_filename = name+'_D.params'
sD_filename = name+'_sD.params'
...
for epoch in range(500):
    ...
    for i, (d, l, sl) in enumerate(train_iter):
        ...
        with autograd.record():   # target discriminator
        ...
        with autograd.record():
            fake_y, sfake_y = GenNet(x)
            with autograd.pause():
                sfake_y = ndarray.argmax(sfake_y, axis=1)
                trans_sfake_y = PredictTrans(sfake_y)
                trans_sreal_y = PredictTrans(sy)

            sreal = DissNet(trans_sreal_y)
            sfake = DissNet(trans_sfake_y)

            sreal_label = nd.ones_like(sreal, ctx=ctx)
            sfake_label = nd.zeros_like(sfake, ctx=ctx)

            errs_real = L2_loss(sreal, sreal_label)
            errs_fake = L2_loss(sfake, sfake_label)
            errDs = (errs_real + errs_fake) * 0.5

        errDs.backward()
        sDtrainer.step(x.shape[0])



        with autograd.record():  # train G
            fake_y,sfake_y = GenNet(x)
            ...
            real = DisNet(fake_y)
            sreal = DissNet(trans_sfake_y)




            errG = L2_loss(real, real_label) + L2_loss(sreal, sreal_label) + 10 * pair_loss

        errG.backward()
        Gtrainer.step(x.shape[0])

        print(
            'Epoch %2d,batch %2d, G_loss %.5f , D_loss %.5f, Ds_loss %.5f , time %.1f sec' % (
                epoch,
                i,
                mx.ndarray.mean(errG).asscalar(),
                mx.ndarray.mean(errD).asscalar(),
                mx.ndarray.mean(errDs).asscalar(),
                time.time() - tic))

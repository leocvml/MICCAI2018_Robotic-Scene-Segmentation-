...
###
#########################################################################################################
class DiceLoss_(mx.gluon.loss.Loss):
    def __init__(self, smooth = 0 , eps = 1e-10, batch_axis=0):
        super(DiceLoss_, self).__init__(None, batch_axis)
        self._smooth = smooth
        self._eps = eps
    def hybrid_forward(self, F, output, label):
        ...
        output_flatten = F.reshape(output,(output.shape[0],output.shape[1],-1))
        label_flatten = F.reshape(label,(label.shape[0],label.shape[1],-1))


        interaction = (output_flatten * label_flatten) + self._smooth
        interaction = F.sum(interaction,axis=2)

        up = 2 * interaction
        down = F.sum(output_flatten, axis=2) + F.sum(label_flatten, axis=2) + self._smooth + self._eps
        diceloss = 1 - (up/down)
        diceloss = F.sum(diceloss, axis=1)
        #print(diceloss.shape)
...
for epoch in range(epochs):
    ...
    for i, (d, l, sl) in enumerate(train_iter):
        ...
        sy = gluon.utils.split_and_load(sl, ctx)


        with autograd.record():
            outputs = [GenNet(X) for X in x]

            with autograd.pause():

                fake_y = [predict2ndimg(output[0]) for output in outputs]
                xfake_y =[mx.ndarray.concat(_x,_fake_y,dim=1) for _x, _fake_y in zip(x, fake_y)]

                real_y = [predict2ndimg(Y) for Y in y]
                x_y =[mx.ndarray.concat(_x,_real_y,dim=1) for _x, _real_y in zip(x, real_y)]


            real = [DisNet(i) for i in x_y]
            fake = [DisNet(i) for i in xfake_y]

            real_label = nd.ones_like(real[0],ctx=ctx)
            fake_label = nd.zeros_like(fake[0],ctx=ctx)

            errD =[( L2_loss(_real, real_label) + L2_loss(_fake, fake_label)) * 0.5 for _real, _fake in zip(real, fake)]

        for l in errD:
            l.backward()
        Dtrainer.step(d.shape[0])

        with autograd.record():
            outputs = [GenNet(X) for X in x]
            pair_loss = [Dice(output[0], _y) + Dice(output[1], _sy) for output, _y, _sy in zip(outputs, y, sy)]

            with autograd.pause():
                fake_y = [predict2ndimg(output[0]) for output in outputs]
                xfake_y = [mx.ndarray.concat(_x, _fake_y, dim=1) for _x, _fake_y in zip(x, fake_y)]

            real = [DisNet(i) for i in xfake_y]


            errG =[ L2_loss(_real, real_label) + 10 * _pair_loss for _real,_pair_loss in zip(real, pair_loss) ]
        ...
        print(
            'Epoch %2d,batch %2d,G_loss %.5f ,D_loss %.5f , time %.1f sec' % (
                ...
                mx.ndarray.mean(errD[0]).asscalar(),
                time.time() - tic))

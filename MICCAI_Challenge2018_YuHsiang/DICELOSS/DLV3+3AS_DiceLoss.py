...
###
#########################################################################################################
class DiceLoss(mx.gluon.loss.Loss):
    def __init__(self, smooth = 0 , eps = 1e-7, batch_axis=0):
        super(DiceLoss, self).__init__(None, batch_axis)
        self._smooth = smooth
        self._eps = eps
    def hybrid_forward(self, F, output, label):
        ...
        output_flatten = F.flatten(output)
        label_flatten = F.flatten(label)

        interaction = (output_flatten * label_flatten) + self._smooth
        interaction = F.sum(interaction,axis=1)

        up = 2 * interaction
        down = F.sum(output_flatten, axis=1) + F.sum(label_flatten, axis=1) + self._smooth + self._eps
        diceloss = 1 - (up/down)
...
for epoch in range(epochs):
    ...
    for i, (d, l, sup_l) in enumerate(train_iter):
        ...
        with autograd.record():
            ...
            errG = [loss(f_y[0], _y) + loss(f_y[1], _sy) for f_y, _y, _sy in zip(fake_y, y, sup_y)]
            # print(len(errG))
        for l in errG:
            l.backward()
